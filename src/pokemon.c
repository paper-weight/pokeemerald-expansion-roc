...
//originally CALC_STAT(base, iv, ev, statIndex, field) and s32 n = (((2 * baseStat + iv + ev / 4) * level) / 100) + 5;    i literally just removed evs from the stat calculation
...
void CalculateMonStats(struct Pokemon *mon)
{
    ...
    else
    {
        ...
        newMaxHP = ((n * level) / 100) + level + 10; //newMaxHP = (((n + hpEV / 4) * level) / 100) + level + 10;
    }
    ...
    CALC_STAT(baseAttack, attackIV, STAT_ATK, MON_DATA_ATK) //CALC_STAT(baseAttack, attackIV, attackEV, STAT_ATK, MON_DATA_ATK)
    CALC_STAT(baseDefense, defenseIV, STAT_DEF, MON_DATA_DEF) //CALC_STAT(baseDefense, defenseIV, defenseEV, STAT_DEF, MON_DATA_DEF)
    CALC_STAT(baseSpeed, speedIV, STAT_SPEED, MON_DATA_SPEED) //CALC_STAT(baseSpeed, speedIV, speedEV, STAT_SPEED, MON_DATA_SPEED)
    CALC_STAT(baseSpAttack, spAttackIV, STAT_SPATK, MON_DATA_SPATK) //CALC_STAT(baseSpAttack, spAttackIV, spAttackEV, STAT_SPATK, MON_DATA_SPATK)
    CALC_STAT(baseSpDefense, spDefenseIV, STAT_SPDEF, MON_DATA_SPDEF) //CALC_STAT(baseSpDefense, spDefenseIV, spDefenseEV, STAT_SPDEF, MON_DATA_SPDEF)
